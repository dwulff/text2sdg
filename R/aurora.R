...
detect_aurora <- function(corpus, sdgs, verbose = FALSE) {
  ...
  if ("SDG-11" %in% sdgs) {
    ...
    if (nrow(s1_hits) > 0) {
      s1_hits <- s1_hits %>%
        dplyr::group_by(doc_id) %>%
        dplyr::filter(
          w_n(token_id[code == "query_1"], token_id[code == "query_2"]),
  ...
  if ("SDG-03" %in% sdgs) {
    ...
    # handle special 2
    s2 <- c(
      "(<neonatal> OR <under-five> OR ( <(<under>) (<5> OR <five>)>~2 ) OR <before fifth>)",
      "(<mortality> OR <death*>)",
    ...
    # handle special 3
    s3 <- c(
      "( <(<tobacco>) (<control*>)>~6 ) OR ( <(<health>) (<smoking>)>~3 )",
